...
static QVector<DomainDef> allDomains()
{
    QVector<DomainDef> d;
    d.append({
...
static QWidget *makeFieldWidget(const FieldDef &f)
{
    if (f.type == QLatin1String("check"))
        return makeCheck(f.label, f.value == QString::fromUtf8("启用"));
    if (f.type == QLatin1String("select"))
        return makeSelectField(f.label, f.value);
    return makeField(f.label, f.value, f.unit);
}

static QWidget *buildDomainPage(const DomainDef &d, QWidget *dialogParent)
{
    auto *root = new QWidget;
    auto *lay = new QVBoxLayout(root);
    lay->setContentsMargins(20, 18, 20, 22);
    lay->setSpacing(14);
    lay->addWidget(makeHeading(d.title, d.subtitle, QString::fromUtf8("配置模板"), d.templates));
    ...
    for (int i = 0; i < d.sections.size(); ++i) {
        const SectionDef &s = d.sections[i];
        auto *sec = new QFrame;
        ...
        for (int fi = 0; fi < s.fields.size(); ++fi)
            fg->addWidget(makeFieldWidget(s.fields[fi]), fi / 2, fi % 2);
        vl->addWidget(fields);
    ...
    sum->layout()->addWidget(makeSummary(d.summary));
    al->addWidget(sum);
    auto *chk = makePanel();
    chk->layout()->addWidget(makePanelTitle(QString::fromUtf8("运行前检查")));
    chk->layout()->addWidget(makeBulletList(d.checks));
    al->addWidget(chk);
    al->addStretch();
    ...
    auto *note = new QLabel(QString::fromUtf8("配置完整度 92% · 4 项运行前检查 · 预计预检查耗时 < 1 min"));
    note->setObjectName(QStringLiteral("NoteLabel"));
    auto *btn = makeButton(QString::fromUtf8("保存并运行预检查"), true);
    wireDummyAction(btn, dialogParent);
    rl->addWidget(dot);
    rl->addWidget(note);
    rl->addStretch();
    rl->addWidget(btn);
...
AnalysisPage::AnalysisPage(QWidget *parent)
    : QWidget(parent)
{
    const QVector<DomainDef> domains = allDomains();
    ...
    for (int i = 0; i < domains.size(); ++i) {
        ...
        sl->addWidget(tab);
        stack->addWidget(wrapScroll(buildDomainPage(domains[i], this)));
    }
